...
import type { HierarchicalMapNode, GenerateMindMapOutput, MindMapData, Note } from '@/lib/types';
...
import { summarizeSelectedNode, SummarizeSelectedNodeOutput } from '@/ai/flows/summarize-selected-node';
...
const buildHierarchy = (nodes: GenerateMindMapOutput['nodes']): HierarchicalMapNode | null => {
  ...
  if (!rootNode && nodes.length > 0) {
    ...
    if (potentialRoots.length > 0) rootNode = nodeMap.get(potentialRoots[0].id)!;
  }
...
const processAndSetMindMapData = (result: GenerateMindMapOutput | null): MindMapData | null => {
...
function WorkspaceContent() {
  ...
  const [selectedNode, setSelectedNode] = useState<HierarchicalMapNode | null>(null);
  const [summary, setSummary] = useState<SummarizeSelectedNodeOutput | null>(null);
  ...
  useEffect(() => {
    if (fetchedMapData) {
      try {
        ...
        const processedData = processAndSetMindMapData(parsedMapData as GenerateMindMapOutput);
  ...
  const handleSave = async (mapDataToSave: MindMapData) => {
    if (!user || !firestore) {
      toast({ variant: 'destructive', title: 'Authentication Error', description: 'You must be logged in to save a mind map.' });
      return false;
    }
    ...
    try {
      ...
      const docRef = doc(firestore, `users/${user.uid}/mindmaps`, mapToSave.mapId);
  ...
  const handleNodeSelect = useCallback(async (node: HierarchicalMapNode | null) => {
  ...
  const handlePdfUpload = async (formData: FormData) => {
    ...
    try {
      ...
      const loadingTask = pdfjsLib.getDocument({ data: fileBuffer, cMapUrl: '/static/cmaps/', cMapPacked: true });
      ...
      toast({
        variant: "destructive",
        title: "Generation Failed",
        description: error.message || "Could not generate mind map from the PDF.",
      });
  ...
  const handleExport = async (format: "json" | "png" | "pdf" | "csv") => {
    if (!mindMapData) {
      toast({ variant: "destructive", title: "Export Failed", description: "There is no mind map to export." });
      return;
    }
    ...
    try {
      switch (format) {
        ...
        case 'png':
          ...
          const dataUrl = await toPng(mindmapContent, { cacheBust: true, filter, style: { transform: 'scale(1)', left: '0', top: '0' } });
        ...
        case 'pdf':
          ...
          const pdfDataUrl = await toPng(pdfContent, { cacheBust: true, pixelRatio: 2, filter, style: { transform: 'scale(1)', left: '0', top: '0' } });
          const { width, height } = pdfContent.getBoundingClientRect();
          const pdf = new jsPDF({ orientation: width > height ? 'landscape' : 'portrait', unit: 'px', format: [width, height] });
          pdf.addImage(pdfDataUrl, 'PNG', 0, 0, pdf.internal.pageSize.getWidth(), pdf.internal.pageSize.getHeight());
          pdf.save(`${title}.pdf`);
          break;
        case 'csv':
      ...
      toast({ title: "Export Successful", description: `Your mind map has been downloaded as a ${format.toUpperCase()} file.` });
    } catch (error: any) {
      console.error(`Failed to export mind map as ${format}:`, error);
      toast({ variant: "destructive", title: "Export Failed", description: error.message || `Could not export the mind map as ${format.toUpperCase()}.` });
    }
  };
  
  const handleManualSave = async () => {
    if (!mindMapData) {
      toast({ variant: 'destructive', title: 'Save Failed', description: 'There is no mind map to save.' });
      return;
    }
          ...
          {isLoading && (
            <div className="absolute inset-0 bg-background/80 flex flex-col items-center justify-center z-20 backdrop-blur-sm">
              ...
              <p className="mt-4 text-lg font-medium text-foreground">
                {isUploading ? 'Analyzing PDF...' : isGenerating ? 'Generating mind map...' : 'Loading mind map...'}
              </p>
              <p className="text-muted-foreground">This may take a moment.</p>
            </div>
          )}
          {mindMapData ? (
            <MindMap data={mindMapData} onNodeSelect={handleNodeSelect} selectedNodeId={selectedNode?.id} />
          ) : !isLoading && (
            <div className="w-full h-full flex flex-col items-center justify-center text-center p-8">
              <Zap className="h-16 w-16 text-muted-foreground/50 mb-4" />
              <h2 className="text-2xl font-headline text-foreground/90">Mind Map Workspace</h2>
              <p className="text-muted-foreground max-w-md mt-2">
                Start by generating a new mind map from text or a PDF, or open an existing one from your dashboard.
              </p>
            </div>
          )}
        </div>
        <SummaryPanel node={selectedNode} onNodeSelect={handleNodeSelect} summary={summary} isLoadingSummary={isLoadingSummary} note={noteForSelectedNode} mindMapData={mindMapData} />
      </div>
      <AIPersona />
    </div>
  );
