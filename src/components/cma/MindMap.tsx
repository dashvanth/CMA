...
export function MindMap({
  ...
  const { nodes, links, width, height } = useMemo(() => {
    if (!data || !data.root) return { nodes: [], links: [], width: 0, height: 0 };
    
    const hierarchy = d3.hierarchy(data.root, d => (d.children && Array.isArray(d.children) ? d.children : []));
    const treeLayout = d3
        ...
        >
          <svg width={width} height={height} className="absolute inset-0 overflow-visible">
            ...
            <rect width="100%" height="100%" fill="url(#grid)" />
             ...
             {links.map((link, i) => {
               const isRelatedToSelected = selectedNodeWithFamily?.family.has(link.source.data.id) && selectedNodeWithFamily?.family.has(link.target.data.id);
               const isDimmed = !!selectedNodeId && !isRelatedToSelected;

              return (
              <g key={`link-group-${i}`} className={cn('transition-all duration-500', isDimmed && 'opacity-30')}>
                 <path
                  d={getPath({
          ...
          <div className="relative w-full h-full">
            {nodes.map((node, i) => (
              <NodeComponent
                key={`${node.data.id}-${i}`}
                node={node}
                onNodeSelect={onNodeSelect as (node: HierarchicalMapNode) => void}
                isSelected={node.data.id === selectedNodeId}
                isDimmed={!!selectedNodeId && !selectedNodeWithFamily?.family.has(node.data.id)}
              />
            ))}
          </div>
        </div>
